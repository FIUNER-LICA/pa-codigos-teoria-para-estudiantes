...
def __obtener_posiciones_validas(posicion: tuple[int, int], n: int) -> list[tuple[int, int]]:
    """
...
def __tablero_completado(tablero: list[list[int]]) -> bool:
    """
...
def __salto_del_caballo_auxiliar(
    ...
    if __tablero_completado(tablero):
        return True

    for posicion in __obtener_posiciones_validas(pos, len(tablero)):
        y, x = posicion

        if tablero[y][x] == 0:
            tablero[y][x] = actual + 1
            if __salto_del_caballo_auxiliar(tablero, posicion, actual + 1):
                return True
            tablero[y][x] = 0
...
def recorrido_del_caballero(n: int) -> list[list[int]]:
    ...
    for i in range(n):
        for j in range(n):
            tablero[i][j] = 1
            if __salto_del_caballo_auxiliar(tablero, (i, j), 1):
                return tablero
            tablero[i][j] = 0
